...
namespace QlikSimpleConnector
{
    ...
    internal class QlikSimpleConnectorConnection : QvxConnection
    {
        private QlikSimpleConnectorServer parent;
        Dictionary<string, string> args = null;


        public QlikSimpleConnectorConnection(QlikSimpleConnectorServer qcs)
            : base()
        {
            QvxLog.SetLogLevels(true, true);

            QvxLog.Log(QvxLogFacility.Application, QvxLogSeverity.Debug, "+ QlikSimpleConnectorConnection()");
            ...
            {
                QvxLog.Log(QvxLogFacility.Application, QvxLogSeverity.Error, "QlikSimpleConnectorConnection() : " + e.Message);
                throw e;
            }
        ...
        public override QvxDataTable ExtractQuery(string query, List<QvxTable> tables)
        {
            ...
            try {
                ...
                IQlikSimpleConnector c = this.parent.Registered(myArgs["qDriver"]);
                ...
                if (m.Success)
                {
                    ...
                    string fromOwner = m.Groups["owner"].Value.Trim();
                    ...
                    string owner = c.getOwners(db, myArgs).Where(ownerItem => ownerItem == fromOwner).FirstOrDefault();
                    if (db != null)
                    {
                        QvxLog.Log(QvxLogFacility.Application, QvxLogSeverity.Debug, "ExtractQuery() : found db!");

                        QvxTable table = c.getTables(db, owner, myArgs).Where(tableItem => tableItem.TableName == fromTable).FirstOrDefault();
                        if (table != null)
                        {
